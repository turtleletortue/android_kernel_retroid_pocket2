...
extern   void led_gpio_output(int pin, int level);
static void musb_id_pin_work(struct work_struct *data)
{
	...
	del_timer_sync(&musb_idle_timer);
		...
		printk("force PHY to idle, 0x6d=%x, 0x6c=%x\n", USBPHY_READ8(0x6d), USBPHY_READ8(0x6c));
		/* wait */
		msleep(55);
		/* restart session */
		devctl = musb_readb(mtk_musb->mregs, MUSB_DEVCTL);
		...
		USBPHY_SET8(0x6d, 0x3e);
		printk("force PHY to host mode, 0x6d=%x, 0x6c=%x\n", USBPHY_READ8(0x6d), USBPHY_READ8(0x6c));
	#endif
...
static irqreturn_t mt_usb_ext_iddig_int(int irq, void *dev_id)
{
	...
	printk("lipeiyang otg eint\n");
	
	
	if (usb_iddig_polarity == IRQF_TRIGGER_HIGH)
	{
		usb_iddig_polarity = IRQF_TRIGGER_LOW;
	}
	else
	{
		usb_iddig_polarity = IRQF_TRIGGER_HIGH;
	}
	//irq_set_irq_type(usb_iddig_number, usb_iddig_polarity);
...
static void otg_int_init(void)
{
	...
	ret = request_irq(usb_iddig_number, mt_usb_ext_iddig_int, IRQF_TRIGGER_LOW, "USB_IDDIG", NULL);
		...
		//printk("otg line [%d] num = [%d]\n",__LINE__,usb_iddig_number);
		//usb_iddig_number = mt_gpio_to_irq(iddig_pin);
		printk("otg line [%d] num = [%d]\n",__LINE__,usb_iddig_number);
		ret = request_irq(usb_iddig_number, mt_usb_ext_iddig_int, IRQF_TRIGGER_LOW, "USB_IDDIG", NULL);
	#endif
	if (ret > 0)
		printk("otg USB IDDIG IRQ LINE not available!!\n");
	else
		printk("otg USB IDDIG IRQ LINE available!!\n");
#else
	u32 phy_id_pull = 0;
