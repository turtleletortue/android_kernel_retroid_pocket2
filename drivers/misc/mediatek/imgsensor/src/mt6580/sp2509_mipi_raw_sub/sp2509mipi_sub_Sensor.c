...
#include "sp2509mipi_sub_Sensor.h"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SP2509MIPI_SUB_SENSOR_ID,
...
static kal_uint32 return_sensor_id(void)
{
	...
	return (1 + ((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03)));
}
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            printk("sp2509 sub sensor id: 0x%x\n", *sensor_id);
...
UINT32 SP2509MIPISubSensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SP2509MIPISubSensorInit    */
