...
#endif
#define PFX "jxh62_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = JXH62MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h  // 0XA062
	
	.checksum_value = 0x523c51f6,		//checksum value for Camera Auto Test
	
	.pre = {
		.pclk = 43200000,				//record different mode's pclk
		.linelength = 1920,				//record different mode's linelength
		.framelength = 750,			//record different mode's framelength
	...
	.i2c_addr_table = {0x60, 0x6c, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x22,0xEE);
	write_cmos_sensor(0x23,0x02);
	write_cmos_sensor(0x24,0x00);
	write_cmos_sensor(0x25,0xD0);
	write_cmos_sensor(0x26,0x25);
	write_cmos_sensor(0x27,0x94);
	write_cmos_sensor(0x28,0x15);
	write_cmos_sensor(0x29,0x02);
	write_cmos_sensor(0x2A,0x43);
	write_cmos_sensor(0x2B,0x21);
	write_cmos_sensor(0x2C,0x08);
	write_cmos_sensor(0x2D,0x01);
	write_cmos_sensor(0x2E,0xBB);
	write_cmos_sensor(0x2F,0xC0);
	write_cmos_sensor(0x41,0x88);
	write_cmos_sensor(0x42,0x12);
	write_cmos_sensor(0x39,0x90);
	write_cmos_sensor(0x1D,0x00);
	write_cmos_sensor(0x1E,0x04);
	write_cmos_sensor(0x7A,0x4C);
	write_cmos_sensor(0x70,0x89);
	write_cmos_sensor(0x71,0x4A);
	write_cmos_sensor(0x72,0x48);
	write_cmos_sensor(0x73,0x43);
	write_cmos_sensor(0x74,0x52);
	...
	write_cmos_sensor(0x30,0x98);
	write_cmos_sensor(0x31,0x0E);
	write_cmos_sensor(0x32,0xF0);
	write_cmos_sensor(0x33,0x4E);
	write_cmos_sensor(0x34,0x2C);
	write_cmos_sensor(0x35,0xA3);
	write_cmos_sensor(0x38,0x40);
	write_cmos_sensor(0x3A,0x08);
	write_cmos_sensor(0x56,0x02);
	write_cmos_sensor(0x60,0x01);
	write_cmos_sensor(0x0D,0x5D); //50
	write_cmos_sensor(0x57,0x80);
	write_cmos_sensor(0x58,0x33);
	write_cmos_sensor(0x5A,0x04);
	write_cmos_sensor(0x5B,0xB6);
	write_cmos_sensor(0x5C,0x08);
	write_cmos_sensor(0x5D,0x67);
	write_cmos_sensor(0x5E,0x04);
	write_cmos_sensor(0x5F,0x08);
	write_cmos_sensor(0x66,0x28);
	write_cmos_sensor(0x67,0xF8);
	write_cmos_sensor(0x68,0x00);
	write_cmos_sensor(0x69,0x74);
	write_cmos_sensor(0x6A,0x1F);
	write_cmos_sensor(0x63,0x80);
	write_cmos_sensor(0x6C,0xC0);
	write_cmos_sensor(0x6E,0x5C);
	write_cmos_sensor(0x82,0x01);
	write_cmos_sensor(0x0C,0x00);
	write_cmos_sensor(0x46,0xC2);
	write_cmos_sensor(0x48,0x7E);
	write_cmos_sensor(0x62,0x40);
	write_cmos_sensor(0x7D,0x57);
	write_cmos_sensor(0x7E,0x28);
	write_cmos_sensor(0x80,0x00);
	write_cmos_sensor(0x4A,0x05);
	write_cmos_sensor(0x49,0x10);
	write_cmos_sensor(0x13,0x81);
	write_cmos_sensor(0x59,0x97);
//	write_cmos_sensor(0x12,0x00);//modified by hwf
	write_cmos_sensor(0x47,0x47);
	mDELAY(500);
	write_cmos_sensor(0x47,0x44);
...
static kal_uint32 open(void)
{
	...
	printk("csh for open the camera");
	//sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect the module used i2c address
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
...
UINT32 JXH62_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	JXH62_MIPI_RAW_SensorInit	*/
