...
char g_cam_name[128] = {0};

void set_global_cam_name(char* name)
{
    //printk("set_global_cam_name: %s\n", name);
    if (strlen(name) > 127) {
        printk("name length > 127, exit\n");
        return;
    }
    if (strstr(g_cam_name, name) == NULL) {
        if (g_cam_name[0] != 0) {
            strcat(g_cam_name, "\n");
        }
        strcat(g_cam_name, name);
    }
    //printk("g_cam_name: %s\n", g_cam_name);
}
//yan end
