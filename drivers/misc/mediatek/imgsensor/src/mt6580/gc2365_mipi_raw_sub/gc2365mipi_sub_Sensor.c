...
#include "gc2365mipi_sub_Sensor.h"
...
static kal_bool GC2365DuringTestPattern = KAL_FALSE;

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2365_SUB_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
       ...
       //  printk("swordking i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, return_sensor_id());
        spin_unlock(&imgsensor_drv_lock);
        do {
            *sensor_id = return_sensor_id();
              printk("gc2365sub  sensor id: 0x%x\n", *sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
              printk("gc2365sub  sensor id: ok\n");
                //yan read device info
                set_global_cam_name("gc2365_mipi_raw(200w)");
                //yan end
                return ERROR_NONE;
            }
              printk("gc2365sub  sensor id: error\n");
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
UINT32 GC2365_MIPI_RAW_Sub_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
