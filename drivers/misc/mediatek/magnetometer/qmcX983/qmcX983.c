...
static ssize_t store_registers_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	sscanf(buf,"0x%x",(unsigned char*)&tempbuf[0]);
...
static ssize_t store_registers_addr(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if(1 == sscanf(buf,"0x%x",(unsigned char*)&regbuf[0])){
...
static long qmcX983_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_QMC_IOCTL_SET_RANGE:
		err = file->f_op->unlocked_ioctl(file, QMC_IOCTL_SET_RANGE, (unsigned long)arg64);
		if (err < 0)
	...
	case COMPAT_QMC_IOCTL_SET_MODE:
		err = file->f_op->unlocked_ioctl(file, QMC_IOCTL_SET_MODE, (unsigned long)arg64);
		if (err < 0)
	...
	case COMPAT_QMC_IOCTL_READ_SENSORDATA:
		err = file->f_op->unlocked_ioctl(file, QMC_IOCTL_READ_SENSORDATA, (unsigned long)arg64);
		if (err < 0)
...
static int qmcX983_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	MSE_LOG("QMCX983_probe 2 - 2, Addr = [%u]\n", temp);
	//dummy = send_data(client, 0x18, 0x80);
	databuf[0] = 0x18;
	databuf[1] = 0x80;
	dummy = I2C_TxData(databuf, 2);
