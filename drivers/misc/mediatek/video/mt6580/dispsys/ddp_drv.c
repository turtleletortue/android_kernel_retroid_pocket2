...
extern char g_lcd_name[64];
extern char g_cam_name[128];
...
static ssize_t disp_kobj_show(struct kobject *kobj, struct attribute *attr, char *buffer)
{
    ...
    //yan read device info
    else if (0 == strcmp(attr->name, "dispinfo"))
        return sprintf(buffer, "%s\n", g_lcd_name);
    //else if (0 == strcmp(attr->name, "caminfo"))
    //    return sprintf(buffer, "%s\n", g_cam_name);
    //yan read flash id for factory_test
    else if(0 == strcmp(attr->name, "emiinfo"))
    {
        char * ptr;
        char flash_name[32] = {0};
        int i = 0;
	...
	else if (1 == strcmp(attr->name, "snled_r_on"))
	{
	    led_gpio_output(1, 1);
	}
	else if (1 == strcmp(attr->name, "snled_r_off"))
	{
	    led_gpio_output(1, 0);
	}
	else if (2 == strcmp(attr->name, "snled_r1_on"))
	{
	    led_gpio_output(2, 1);
	}
	else if (2 == strcmp(attr->name, "snled_r1_off"))
	{
	    led_gpio_output(2, 0);
	}
	else if (3 == strcmp(attr->name, "snled_r2_on"))
	{
	    led_gpio_output(3, 1);
	}
	else if (3 == strcmp(attr->name, "snled_r2_off"))
	{
	    led_gpio_output(3, 0);
	}	
    //yan end
...
static struct kobj_type disp_kobj_ktype = {
	...
	.default_attrs = (struct attribute *[]){
                        ...
                        //yan read device info
                        &(struct attribute){ 
                                .name = "dispinfo",   //lcd id
                                .mode = S_IRUGO},
                        &(struct attribute){ 
                                .name = "caminfo",   //camera info
                                .mode = S_IRUGO},
                        &(struct attribute){ 
                                .name = "emiinfo",   //flash info
                                .mode = S_IRUGO},
                        //yan end
						&(struct attribute){
							.name = "snled_r_on", 
