...
int APDS9930_irq_registration(struct i2c_client *client)
{
	...
	g_APDS9930_ptr = obj;
		...
		APS_ERR("csh   APDS9930_irq_registration!\n");
 ...
 /*   enable_irq(alsps_irq);*/
 		...
 		APS_ERR("csh 22222  APDS9930_irq_registration!\n");
	return ret;
...
static int APDS9930_get_ps_value(struct APDS9930_priv *obj, u16 ps)
{
	...
	printk("APDS9930_get_ps_value init!\n");
	if (ps_cali.valid == 1) {
		printk("APDS9930_get_ps_value ps_cali.valid == 1!\n");
		if (ps > ps_cali.close) {
			printk("APDS9930_get_ps_value ps > ps_cali.close!\n");
			val = 0;	/*close */
			val_temp = 0;
			intr_flag_value = 1;
		}
			...
			printk("APDS9930_get_ps_value ps < ps_cali.far_away!\n");
			val = 1;	/*far away */
			val_temp = 1;
			intr_flag_value = 0;
		} else
		...
		if (ps > atomic_read(&obj->ps_thd_val_high)) {
			printk("APDS9930_get_ps_value ps > atomic_read(&obj->ps_thd_val_high)!\n");
			printk("APDS9930_get_ps_value ps=%d, &obj->ps_thd_val_high=%d\n",ps, atomic_read(&obj->ps_thd_val_high));
			val = 0;	/*close */
			val_temp = 0;
			intr_flag_value = 1;
		} else if (ps < atomic_read(&obj->ps_thd_val_low)) {
			printk("APDS9930_get_ps_value ps < atomic_read(&obj->ps_thd_val_low)!\n");
			printk("APDS9930_get_ps_value ps=%d, &obj->ps_thd_val_low=%d\n",ps, atomic_read(&obj->ps_thd_val_low));
			val = 1;	/*far away */
			val_temp = 1;
			intr_flag_value = 0;
		} else
	...
	if (!invalid) {
		/* APS_DBG("PS:  %05d => %05d\n", ps, val); */
		printk("APDS9930_get_ps_value val=%d\n",val);
		return val;
	} else {
		printk("APDS9930_get_ps_value invalid\n");
		return -1;
	}
}
		...
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			...
			if (value) {
				...
				if (err) {
					...
					APS_ERR("[leung]enable ps success!\n");
				}
				set_bit(CMC_BIT_PS, &obj->enable);
#if 1
				if (!test_bit(CMC_BIT_ALS, &obj->enable)) {
					...
					APS_ERR("[leung]enable als success!\n");
				}
				}
#endif
			} else {
...
*/
	...
	{
		client->addr = 0x39;
		printk("frank_zhonghua:APDS9930 client->addr=%d\n",client->addr);
	}
	//add end
