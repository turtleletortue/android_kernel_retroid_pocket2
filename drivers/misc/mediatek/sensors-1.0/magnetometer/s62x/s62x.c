...
static int s62x_set_delay(u64 ns)
{
	int value = 0;
...
static int s62x_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return 0;
}

static int s62x_flush(void)
{
	return mag_flush_report();
}

static int s62x_open_report_data(int open)
{
	return 0;
}

static int s62x_get_data(int *x , int *y, int *z, int *status)
{
	mutex_lock(&sensor_data_mutex);
			  ...
			  const struct i2c_device_id *id)
{
	...
	struct s62x_i2c_data *data = NULL;
	int err = 0;
	...
	ctl.enable = s62x_enable;
	ctl.set_delay  = s62x_set_delay;
	ctl.open_report_data = s62x_open_report_data;
	ctl.batch = s62x_batch;
	ctl.flush = s62x_flush;
	...
	mag_data.get_data = s62x_get_data;
