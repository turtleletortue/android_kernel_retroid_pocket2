...
static long retroid_input_ioctl(struct file *, unsigned int, unsigned long);
...
static int is_standby;

extern unsigned int retroid_tl3_state;
extern unsigned int retroid_tr3_state;
	...
	while (retroid_gpios_ptr->pin > 0) {
		switch (retroid_gpios_ptr->pin) {
			case BTN_VOLUME_DOWN_VIRTUAL_GPIO_PIN:
				retroid_gpios_ptr->state = retroid_tl3_state;
				break;
			case BTN_VOLUME_UP_VIRTUAL_GPIO_PIN:
				retroid_gpios_ptr->state = retroid_tr3_state;
				break;
			default:
...
static long retroid_input_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    switch (cmd) {
...
static ssize_t retroid_standby_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	if (!kstrtouint(buf, 0, &is_standby))
		return count;
	return 0;
}

static ssize_t retroid_standby_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count;
	count = sprintf(buf, "standby: %d\n", is_standby);
	return count;
}

static DEVICE_ATTR(getinfo, 0664, retroid_getinfo_show, NULL);
static DEVICE_ATTR(standby, 0666, retroid_standby_show, retroid_standby_store);
