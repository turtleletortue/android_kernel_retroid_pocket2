...
unsigned int retroid_tl3_state = 0;
unsigned int retroid_tr3_state = 0;
...
static unsigned int joystick_mode = 0;
...
static ssize_t kpd_store_joystick(struct device_driver *ddri, const char *buf, size_t count)
{
	int ret;
	ret = kstrtouint(buf, 0, &joystick_mode);
	printk("kpd_store_joystick %d\n",joystick_mode);
	return count;
}

static DRIVER_ATTR(kpd_call_state, S_IWUSR | S_IRUGO, kpd_show_call_state, kpd_store_call_state);
static DRIVER_ATTR(joystick, 0664, NULL, kpd_store_joystick);

static struct driver_attribute *kpd_attr_list[] = {
	&driver_attr_kpd_call_state,
	&driver_attr_joystick,
};
		...
		for (j = 0; j < 16; j++) {
			...
			if(joystick_mode){
				if (linux_keycode == 115)
					retroid_tr3_state = pressed;
				if (linux_keycode == 114)
					retroid_tl3_state = pressed;
			}else{
...
static int kpd_open(struct input_dev *dev)
{
    ...
    //mt_set_gpio_mode( GPIO_KPD_KCOL1_PIN, 1 );
    //mt_set_gpio_pull_select(GPIO_KPD_KCOL1_PIN, 1);
	return 0;
}
...
{
	kpd_suspend = true;
//#ifdef MTK_KP_WAKESOURCE //leung modify
	if (call_status == 2) {
		kpd_print("kpd_early_suspend wake up source enable!! (%d)\n", kpd_suspend);
	} else {
		//kpd_wakeup_src_setting(0); //leung modify
