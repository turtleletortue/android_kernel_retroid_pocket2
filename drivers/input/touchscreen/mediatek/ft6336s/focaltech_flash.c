...
int fts_ctpm_fw_upgrade_ReadVendorID(struct i2c_client *client, u8 *ucPVendorID)
{
	...
	int i_ret;
	int FTS_UPGRADE_LOOP1 = 21;
	unsigned char au_delay_timing[21] = {30, 33, 36, 39, 42, 45, 48, 51,54,57,60,28,26,24,22,20,18,16,14,12,10};
	...
	*ucPVendorID = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP1; i++) 
	{
		/********* Step 1:Reset  CTPM *****/
		fts_write_reg(client, 0xfc, FTS_UPGRADE_AA);
		msleep(au_delay_timing[i]);
		...
		if ((reg_val[0] == 0x79) && (reg_val[1] == 0x18)) {
			memcpy(&fts_updateinfo_curr, &fts_updateinfo[4], sizeof(struct fts_Upgrade_Info));
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
        ...
        reg_addr = FTS_REG_FW_VENDOR_ID;
        err = fts_i2c_read(client, &reg_addr, 1, &fw_vendor_id, 1);
        if (err < 0)
                dev_err(&client->dev, "fw vendor id read failed");
	...
	//	) 
	if ((0x51!=fw_vendor_id) || (uc_tp_fm_ver < uc_host_fm_ver)) 	// avoid auto upgrade fail, it will lead to read/write reg fail
	{
		#if FT_TP
