...
unsigned int touch_irq = 0;//add lihonghua
...
int i2c_read_bytes_non_dma(struct i2c_client *client, u16 addr, u8 *rxbuf, int len)
{
    ...
    {
        {
            ...
            //.addr = ((client->addr &I2C_MASK_FLAG) | (I2C_PUSHPULL_FLAG)),//lihonghua
            .flags = 0,
            .buf = buffer,
            .len = GTP_ADDR_LENGTH,
            .timing = I2C_MASTER_CLOCK
        ...
        {
            .addr = ((client->addr &I2C_MASK_FLAG) | (I2C_ENEXT_FLAG)),
            //.addr = ((client->addr &I2C_MASK_FLAG) | (I2C_PUSHPULL_FLAG)),//add lihonghua
            .flags = I2C_M_RD,
            .timing = I2C_MASTER_CLOCK
        },
    };
	...
	//printk("lihonghua2_i2c_read_bytes to device %02X address %04X len %d\n", client->addr, addr, len);
...
int i2c_write_bytes_non_dma(struct i2c_client *client, u16 addr, u8 *txbuf, int len)
{
    ...
    {
        .addr = ((client->addr &I2C_MASK_FLAG) | (I2C_ENEXT_FLAG)),
        //.addr = ((client->addr &I2C_MASK_FLAG) | (I2C_PUSHPULL_FLAG)),//modify by lihonghua
        .flags = 0,
        .buf = buffer,
        .timing = I2C_MASTER_CLOCK,
    };
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
   ...
   while (retry++ < 5)
	 //while (1)
    {
        ...
        {
			GTP_ERROR("GTP_REG_HW_INFO i2c success");
            return ret;
        }
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    msleep(ms);
    GTP_GPIO_OUTPUT(GTP_INT_PORT, client->addr == 0x14);//modify lihonghua
	...
	if (node) {
		...
		ret = request_irq(touch_irq, (irq_handler_t)tpd_eint_interrupt_handler,
			...
			if (ret > 0)
				...
				TPD_DMESG("[leung] tpd request_irq IRQ ret=%d,touch_irq=%d\n", ret, touch_irq);
	} else {
...
static int tpd_power_on(struct i2c_client *client)
{
	...
	/*
	if(i2c_client->addr != 0x5D)
	{
		i2c_client->addr = 0x5D;
...
//	struct device_node *node = NULL;//add lihonghua
	//u32 ints[2] = { 0, 0 };//add lihonghua
	s32 err = 0;
    s32 ret = 0;
...
#endif*/
    //printk("lihonghua tpd_read_version.\n");
...
{
	//printk("lihonghua tpd_local_init\n");
	int retval;
...
//		printk("regulator_enable() failed!\n");	
    ...
    memset(gpDMABuf_va, 0, GTP_DMA_MAX_TRANSACTION_LENGTH);*///modify lihonghua
	tpd->dev->dev.coherent_dma_mask = DMA_BIT_MASK(32);
		gpDMABuf_va = (u8 *) dma_alloc_coherent(&tpd->dev->dev, IIC_DMA_MAX_TRANSFER_SIZE,
		//gpDMABuf_va = (u8 *) dma_alloc_coherent(&tpd->dev->dev, 4096,//modify lihonghua
    ...
    //printk("lihonghua tpd_local_init out.\n"); 
...
static s8 gtp_enter_sleep(struct i2c_client *client)
{
...
#else
    {
     ...
     GTP_ERROR("before regulator_disable\n");
   //     GTP_GPIO_OUTPUT(GTP_INT_PORT, 0);
	...
	//csh
		...
		if (ret != 0)
			...
			GTP_ERROR("regulator_disable success\n");
		}
		tpd_gpio_output(tpd_rst_gpio_number, 0);
	//end
...
static void tpd_suspend(struct device *h)
{
...
#else
    //mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);//modify lihonghua
    ...
    if (ret < 0)
    {
		...
		TPD_DMESG("tpd_suspend early suspend success\n");
	}
...
static void tpd_resume(struct device *h)
{
	...
	if (ret != 0)
		...
		GTP_ERROR("regulator_enable success\n");
	}
...
/*
    ...
    if (ret < 0)
    {
		...
		TPD_DMESG("tpd_resume later resume success\n");
	}
	*/
    ...
    doze_status = DOZE_DISABLED;
#else 
    //mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);//modify lihonghua
...
{
    GTP_INFO("MediaTek gt9157 touch panel driver init\n");
	tpd_get_dts_info();
/*#ifdef MT6572
