...
#ifdef CONFIG_FT6X36_PROXIMITY
#include <hwmsensor.h>
#include <hwmsen_dev.h>
#include <sensors_io.h>
#endif

#define FTS_SETTING_DMA_BUF_LEN 128//250
...
#define CONFIG_FT6X36_PROXIMITY_DEBUG(fmt,arg...) printk("<<proximity>> "fmt"\n",##arg)
#define CONFIG_FT6X36_PROXIMITY_DMESG(fmt,arg...) printk("<<proximity>> "fmt"\n",##arg)
...
//#define __MSG_DMA_MODE__
	...
	static void msg_dma_alloct(void)
	{
	    if (NULL == g_dma_buff_va)
    		{
    		      #ifdef CONFIG_64BIT
       		 tpd->dev->dev.coherent_dma_mask = DMA_BIT_MASK(64);
       		 g_dma_buff_va = (u8 *)dma_alloc_coherent(&tpd->dev->dev, FTS_SETTING_DMA_BUF_LEN, &g_dma_buff_pa, GFP_KERNEL);
       		#else
       		 tpd->dev->dev.coherent_dma_mask = DMA_BIT_MASK(32);
       		 //g_dma_buff_va = (u8 *)dma_alloc_coherent(NULL, FTS_SETTING_DMA_BUF_LEN, &g_dma_buff_pa, GFP_KERNEL);
			 g_dma_buff_va = (u8 *)dma_alloc_coherent(&tpd->dev->dev, FTS_SETTING_DMA_BUF_LEN, &g_dma_buff_pa, GFP_KERNEL);
       		#endif
    		}
	...
	static void msg_dma_release(void){
		...
		{
		     #ifdef CONFIG_64BIT
			dma_free_coherent(&tpd->dev->dev, FTS_SETTING_DMA_BUF_LEN, g_dma_buff_va, g_dma_buff_pa);
                   #else
			dma_free_coherent(NULL, FTS_SETTING_DMA_BUF_LEN, g_dma_buff_va, g_dma_buff_pa);
	     	      #endif
	        	g_dma_buff_va = NULL;
	        	g_dma_buff_pa = 0;
...
static const struct i2c_device_id ft5x0x_tpd_id[] = {{"ft6x36", 0}, {} };
static const struct of_device_id ft5x0x_dt_match[] = {
	{.compatible = "mediatek,cap_touch"},
//	{.compatible = "malata,ft_touch"},
	{},
};
...
static int tpd_enable_ps(int enable)
{
	...
	if (enable){
		...
		CONFIG_FT6X36_PROXIMITY_DEBUG("[proxi_5206]ps function is on\n");	
	}else{
		state &= 0x00;	
		tpd_proximity_flag = 0;
		CONFIG_FT6X36_PROXIMITY_DEBUG("[proxi_5206]ps function is off\n");
	}

	ret = i2c_smbus_write_i2c_block_data(i2c_client, 0xB0, 1, &state);
	CONFIG_FT6X36_PROXIMITY_DEBUG("[proxi_5206]write: 0xB0's value is 0x%02X\n", state);
	return 0;
}
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				else
				{
					...
					CONFIG_FT6X36_PROXIMITY_DEBUG("huang sensor_data->values[0] 1082 = %d\n", sensor_data->values[0]);
...
int fts_i2c_read(struct i2c_client *client, char *writebuf,int writelen, char *readbuf, int readlen)
{
	...
	if((NULL!=client) && (writelen>0) && (writelen<=FTS_SETTING_DMA_BUF_LEN))
	{
		// DMA Write
	...
	if((NULL!=client) && (readlen>0) && (readlen<=FTS_SETTING_DMA_BUF_LEN))
...
static int touch_event_handler(void *unused)
{
	...
	//	printk("llw tp wait --\n");
		...
		{
			i2c_smbus_read_i2c_block_data(i2c_client, 0xB0, 1, &ps_state);
			CONFIG_FT6X36_PROXIMITY_DEBUG("proxi_5206 0xB0 state value is 1131 0x%02X\n", ps_state);
			...
			CONFIG_FT6X36_PROXIMITY_DEBUG("proxi_5206 0x01 value is 1139 0x%02X\n", proximity_status);
			...
			CONFIG_FT6X36_PROXIMITY_DEBUG("tpd_proximity_detect 1149 = %d\n", tpd_proximity_detect);

			if ((err = tpd_read_ps()))
			{
				CONFIG_FT6X36_PROXIMITY_DMESG("proxi_5206 read ps data 1156: %d\n", err);	
			}
			sensor_data.values[0] = tpd_get_ps_value();
			sensor_data.value_divide = 1;
			sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
			...
			{
				CONFIG_FT6X36_PROXIMITY_DMESG(" proxi_5206 call hwmsen_get_interrupt_data failed= %d\n", err);	
			}
		}  
#endif
...
#ifdef TPD_AUTO_UPGRADE
static int tpd_firmware_upgrade_thread(void *unused)
{
	int err = 0;
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		TPD_DMESG("%s: i2c check functionality error\n", __func__);
		return  -ENODEV;
	}
	...
	TPD_DMESG("mtk_tpd: tpd_probe ft6x36\n");
...
reset_proc:
	...
	if(err< 0 || data!=0)// reg0 data running state is 0; other state is not 0
	{
		...
		if(gpDMABuf_va)
		{
			dma_free_coherent(&client->dev, 250, tpd_i2c_dma_va, (dma_addr_t)tpd_i2c_dma_pa);
			tpd_i2c_dma_va = NULL;
			tpd_i2c_dma_pa = NULL;
		}
#endif
...
*/
	...
	msleep(400);
		...
		pKThread = kthread_run(tpd_firmware_upgrade_thread, 0, TPD_DEVICE);//modify zhjg 20160329
	#endif
...
//extern bool mt65xx_check_is_right_lcm(char * ven_name);
...
static int __init tpd_driver_init(void)
{
    ...
    /*
	 if(!mt65xx_check_is_right_lcm("lide_hx8394f"))
	 	return -1;
    */
