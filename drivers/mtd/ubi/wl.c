...
static void wl_set_type(struct ubi_device *ubi, struct ubi_wl_entry *e, struct rb_root *root)
{
	if (root == &ubi->used)
		...
		ubi_err("PEB %d add to wrang list address 0x%p", e->pnum, root);
}
...
retry:
	if (!ubi->free.rb_node) {
		if (ubi->works_count == 0) {
			...
			ubi_err("archive count:%d %p", ubi->tlc_archive_count, ubi->archive.rb_node);
	...
	if (!e) {
		...
		ubi_err("free count:%d", ubi->free_count);
		ubi_err("archive count:%d", ubi->tlc_archive_count);
		return -ENOSPC;
	}
				...
				int shutdown)
{
	...
	ubi_msg("wl schedule %lu archive %d work %d slc free %d",
		ubi->wl_scheduled, ubi->tlc_archive_count, ubi->works_count, ubi->free_count);
	/*ubi_err("2");*/
	if (wl_mode == WL_MODE_SCRUB) {
		...
		if (e1->tlc) {
			if (ubi->tlc_free.rb_node == NULL) {
				ubi_err("no free tlc peb cancel scrub tlc PEB %d", e1->pnum);
				goto out_cancel;
			}
			e2 = get_peb_for_tlc_wl(ubi);
		} else {
			if (ubi->free.rb_node == NULL) {
				ubi_err("no free peb cancel scrub slc PEB %d", e1->pnum);
				goto out_cancel;
			}
		...
		if (ubi->tlc_free.rb_node == NULL) {
			ubi_err("no free tlc peb cancel tlc wl PEB %d", e1->pnum);
			goto out_cancel;
		}
		e2 = get_peb_for_tlc_wl(ubi);
		if (e2->ec - e1->ec < ubi->tlc_wl_th) {
			ubi_err("no need tlc wl e1->ec %d, e2->ec %d", e1->ec, e2->ec);
		...
		ubi_msg("tlc wl move PEB %d EC %d to PEB %d EC %d", e1->pnum, e1->ec, e2->pnum, e2->ec);
	} else if (wl_mode == WL_MODE_ARCHIVE) {
		if (!ubi->archive.rb_node)
			goto out_cancel;
		e1 = rb_entry(rb_first(&ubi->archive), struct ubi_wl_entry, u.rb);
		if (ubi->tlc_free.rb_node == NULL) {
			ubi_err("no free tlc peb cancel archive PEB %d", e1->pnum);
			goto out_cancel;
		}
		e2 = get_peb_for_tlc_wl(ubi);
		archiving = 1;
		...
		if (ubi->free.rb_node == NULL) {
			ubi_err("no free slc peb cancel wl PEB %d", e1->pnum);
			goto out_cancel;
		}
		e2 = get_peb_for_wl(ubi);
		if (e2->ec - e1->ec < ubi->wl_th) {
			ubi_err("no need slc wl e1->ec %d, e2->ec %d", e1->ec, e2->ec);
		...
		ubi_msg("slc wl move PEB %d EC %d to PEB %d EC %d", e1->pnum, e1->ec, e2->pnum, e2->ec);
	} else
		ubi_err("Wrong wl mode %d", wl_mode);
	/*ubi_err("6");*/
	...
	if (err && err != UBI_IO_BITFLIPS) {
		if (err == UBI_IO_FF) {
			 ...
			 * move it.
			 */
			ubi_err("PEB %d has no VID header", e1->pnum);
			protect = 1;
			erase_e2 = 0;
			goto out_not_moved;
...
int ubi_found_blocks_to_archive(struct ubi_device *ubi, struct rb_root *root)
{
	struct rb_node *p;
	struct ubi_wl_entry *e = NULL;
	int i = 0, ret;
	...
	if (ubi->attaching == 1)
		ubi_msg("forcely scan when booting");
	...
	if (mtbl_vol->eba_tbl)
		mtbl_pnum = mtbl_vol->eba_tbl[0];

	vol_vol = ubi->volumes[vol_id2idx(ubi, UBI_LAYOUT_VOLUME_ID)];
	if (vol_vol->eba_tbl) {
		vol_pnum[0] = vol_vol->eba_tbl[0];
		vol_pnum[1] = vol_vol->eba_tbl[1];
	}
	...
	ubi_rb_for_each_entry(p, e, root, u.rb) {
		if (e->pnum == mtbl_pnum)
			continue;
		if (e->pnum == vol_pnum[0])
			continue;
		if (e->pnum == vol_pnum[1])
			continue;
		pebs[i] = e->pnum;
		i++;
		if(i==MAX_CHECK_PEBS)
	...
	for (i = 0; i < MAX_CHECK_PEBS; i++) {
		...
		if (ret != 0) {
			ubi_err("ubi_io_read_data ret %d", ret);
		...
		if (data_size < min_data_size) {
			...
			min_peb = peb;
		} else if (data_size == ubi->leb_size) {
			full++;
			ubi_msg("archive PEB %d", peb);
			__ubi_wl_archive_leb(ubi, peb);
		}
	}
	if (full == 0 && min_peb != -1) {
		ubi_msg("archive PEB %d data size %d", min_peb, min_data_size);
		__ubi_wl_archive_leb(ubi, min_peb);
	}
	vfree(pebs);
	return 0;
}
...
static int ensure_wear_leveling(struct ubi_device *ubi, int nested)
{
	...
	ubi_found_blocks_to_archive(ubi, &ubi->used);
#endif
...
retry:
	...
	ubi_msg("leb %d:%d e->pnum %d type %d pnum %d, tlc %d\n", vol_id, lnum, e->pnum, e->type, pnum, e->tlc);
#ifdef CONFIG_MTK_SLC_BUFFER_SUPPORT
	if (ubi_remove_wl_tree(ubi, e)) {
		ubi_msg("ubi_remove_wl_tree %d", e->type);
...
int ubi_wl_archive_leb(struct ubi_device *ubi, struct ubi_volume *vol, int lnum)
{
	...
	err = __ubi_wl_archive_leb(ubi, pnum);
	return err;
...
int ubi_wipe_mtbl_record(struct ubi_device *ubi, int vol_id)
{
	...
	ubi_err("magic: %x", ubi->mtbl->magic);
	ubi_err("crc: %x", ubi->mtbl->crc);
	ubi_err("peb_count: %x", ubi->mtbl->peb_count);
	ubi_err("ec:");
	for (i = 0; i < ubi->mtbl_slots; i++) {
