		      ...
		      const void *buf, int offset, int len)
{
	...
	if (pnum >= 0) {
		...
		if (ubi_peb_istlc(ubi, pnum) == 1) {
			...
			ubi_err("write LEB %d tlc PEB %d offset %d size %d", lnum, pnum, offset, len);
			...
			for (i = offset ; i < offset + len ; i++)
				if (p[i] != 0xFF)
					ubi_err("LINGER: last page offset %d is not empty %d\n", i, p[i]);
		...
		struct ubi_vid_hdr *vid_hdr, int do_wl)
{
	...
	if (1) {
		...
		if (crc != crc32(UBI_CRC32_INIT, leb_buf, data_size)) {
			...
			ubi_warn("1 read data back from PEB %d and it is different", to);
			...
			if (crc != crc32(UBI_CRC32_INIT, tmp_buf, data_size)) {
				ubi_warn("2 read data back from PEB %d and it is different",
						to);
			...
			for (i = 0; i < aldata_size; i++) {
				...
				if (*a != *b)
					ubi_err("i %d, tmp_buf %d, leb_buf %d\n", i, *a, *b);
			}
			vfree(tmp_buf);
		}
#endif
