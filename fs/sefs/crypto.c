				  ...
				  char *src, int len)
{
	...
	if (!desc.tfm) {
		desc.tfm = crypto_alloc_hash(SEFS_DEFAULT_HASH, 0,
					     CRYPTO_ALG_ASYNC);
		if (IS_ERR(desc.tfm)) {
			rc = PTR_ERR(desc.tfm);
 ...
 */
void
sefs_init_crypt_stat(struct sefs_crypt_stat *crypt_stat)
{
	...
	struct sefs_mount_crypt_stat *mount_crypt_stat)
{
	...
	list_for_each_entry_safe(auth_tok, auth_tok_tmp,
		...
		if (auth_tok->global_auth_tok_key
		    && !(auth_tok->flags & SEFS_AUTH_TOK_INVALID))
 ...
 */
int sefs_virt_to_scatterlist(const void *addr, int size, struct scatterlist *sg,
			int sg_size)
{
	int i = 0;
	struct page *pg;
 ...
 *
 * Convert an Sefs page index into a lower byte offset
 */
...
/**
 * sefs_encrypt_page
 * @page: Page mapped from the Sefs inode for the file; contains
 *        decrypted content that needs to be encrypted (to a temporary
 *        page; not in place) and written out to the lower file
 *
 * Encrypt an Sefs page. This is done on a per-extent basis. Note
 * that Sefs pages may straddle the lower pages -- for instance,
 * if the file was created on a machine with an 8K page size
 * (resulting in an 8K header), and then the file is copied onto a
 * host with a 32K page size, then when reading page 0 of the Sefs
...
/**
 * sefs_decrypt_page
 * @page: Page mapped from the Sefs inode for the file; data read
 *        and decrypted from the lower file will be written into this
 *        page
 *
 * Decrypt an Sefs page. This is done on a per-extent basis. Note
 * that Sefs pages may straddle the lower pages -- for instance,
 * if the file was created on a machine with an 8K page size
 * (resulting in an 8K header), and then the file is copied onto a
 * host with a 32K page size, then when reading page 0 of the Sefs
...
int sefs_decrypt_page(struct page *page)
{
	...
	lower_offset = lower_offset_for_page(crypt_stat, page);
	page_virt = kmap(page);
	rc = sefs_read_lower(page_virt, lower_offset, PAGE_CACHE_SIZE,
				 sefs_inode);
	kunmap(page);
	if (rc < 0) {
		sefs_printk(KERN_ERR,
...
void sefs_set_default_sizes(struct sefs_crypt_stat *crypt_stat)
{
	...
	if (crypt_stat->flags & SEFS_METADATA_IN_XATTR)
		...
		if (PAGE_CACHE_SIZE <= SEFS_MINIMUM_HEADER_EXTENT_SIZE)
				 ...
				 char *page_virt, size_t size)
{
	...
	rc = sefs_setxattr(sefs_dentry, SEFS_XATTR_NAME, page_virt,
			       size, 0);
	return rc;
}
...
/**
 * sefs_write_metadata
 * @sefs_dentry: The Sefs dentry, which should be negative
 * @sefs_inode: The newly created Sefs inode
 *
			    ...
			    struct inode *sefs_inode)
{
	...
	if (crypt_stat->flags & SEFS_METADATA_IN_XATTR)
		rc = sefs_write_metadata_to_xattr(sefs_dentry, virt,
						      size);
	else
				      ...
				      int validate_header_size)
{
	...
	if (!(crypt_stat->flags & SEFS_I_SIZE_INITIALIZED))
		sefs_i_size_init(page_virt, sefs_dentry->d_inode);
...
int sefs_read_xattr_region(char *page_virt, struct inode *sefs_inode)
{
	...
	size = sefs_getxattr_lower(lower_dentry, SEFS_XATTR_NAME,
...
int sefs_read_metadata(struct dentry *sefs_dentry)
{
	int rc;
	char *page_virt;
	struct inode *sefs_inode = sefs_dentry->d_inode;
	struct sefs_crypt_stat *crypt_stat =
	    &sefs_inode_to_private(sefs_inode)->crypt_stat;
	struct sefs_mount_crypt_stat *mount_crypt_stat =
		&sefs_superblock_to_private(
	...
	if (rc) {
		...
		if (rc) {
			printk(KERN_DEBUG "Valid Sefs headers not found in "
		...
		if (crypt_stat->mount_crypt_stat->flags
			...
			printk(KERN_WARNING "Attempt to access file with "
			       ...
			       "region, but Sefs was mounted without "
			       "xattr support enabled. Sefs will not treat "
...
/**
 * sefs_encrypt_filename - encrypt filename
 *
 ...
 */
static int
sefs_encrypt_filename(struct sefs_filename *filename,
			  struct sefs_crypt_stat *crypt_stat,
			  struct sefs_mount_crypt_stat *mount_crypt_stat)
{
	int rc = 0;
	...
	if ((crypt_stat && (crypt_stat->flags & SEFS_ENCFN_USE_MOUNT_FNEK))
	    || (mount_crypt_stat && (mount_crypt_stat->flags
				     & SEFS_GLOBAL_ENCFN_USE_MOUNT_FNEK))) {
		size_t packet_size;
		size_t remaining_bytes;

		rc = sefs_write_tag_70_packet(
			NULL, NULL,
		...
		rc = sefs_write_tag_70_packet(filename->encrypted_filename,
						  &remaining_bytes,
						  &packet_size,
						  mount_crypt_stat,
						  filename->filename,
 ...
 */
static int
sefs_process_key_cipher(struct crypto_blkcipher **key_tfm,
...
struct mutex sefs_key_tfm_list_mutex;

int __init sefs_init_crypto(void)
{
	mutex_init(&sefs_key_tfm_list_mutex);
...
int sefs_destroy_crypto(void)
{
	...
	mutex_lock(&sefs_key_tfm_list_mutex);
	...
	mutex_unlock(&sefs_key_tfm_list_mutex);
	return 0;
}
			 ...
			 size_t key_size)
{
	...
	BUG_ON(!mutex_is_locked(&sefs_key_tfm_list_mutex));
 ...
 * Must be called with &sefs_key_tfm_list_mutex held
 ...
 */
int sefs_get_tfm_and_mutex_for_cipher_name(struct crypto_blkcipher **tfm,
...
static size_t sefs_max_decoded_size(size_t encoded_size)
{
 ...
 */
static void
sefs_decode_from_filename(unsigned char *dst, size_t *dst_size,
			      const unsigned char *src, size_t src_size)
{
	...
	if (dst == NULL) {
		(*dst_size) = sefs_max_decoded_size(src_size);
		goto out;
	}
	...
	const char *name, size_t name_size)
{
	...
	(*encoded_name) = NULL;
	(*encoded_name_size) = 0;
	if ((crypt_stat && (crypt_stat->flags & SEFS_ENCRYPT_FILENAMES))
	    || (mount_crypt_stat && (mount_crypt_stat->flags
		...
		#if 1
		printk("sefs name_size:%d pos1:%d pos2:%d pos3:%d byte1:%c bypte2:%c byte3:%c rand1:%d rand2:%d name[pos1]:%c name[pos2]:%c name[pos3]:%c\n",
			   name_size,pos1,pos2,pos3,byte1,byte2,byte3,rand1,rand2,name[pos1],name[pos2],name[pos3]);
		#endif
	} else {
 ...
 * @sefs_dir_dentry: Sefs directory dentry
