...
{
	struct inode *inode = sefs_get_inode(lower_dentry->d_inode, sb);
 ...
 * @nd: nameidata of sefs' parent's dentry & vfsmount
 *
 * Creates the underlying file and the Sefs inode which will link to
 * it. It will also update the Sefs directory inode to mimic the
 * stat of the lower directory inode.
 *
 * Returns the new Sefs inode on success; an ERR_PTR on error condition
 */
static struct inode *
		   ...
		   struct dentry *sefs_dentry, umode_t mode)
{
	...
	inode = __sefs_get_inode(lower_dentry->d_inode,
		...
		umode_t mode, bool excl)
{
	...
	if (rc) {
		sefs_do_unlink(directory_inode, sefs_dentry,
				   sefs_inode);
		make_bad_inode(sefs_inode);
		unlock_new_inode(sefs_inode);
		iput(sefs_inode);
		goto out;
	}
	unlock_new_inode(sefs_inode);
	d_instantiate(sefs_dentry, sefs_inode);
out:
	return rc;
}
...
static int sefs_i_size_read(struct dentry *dentry, struct inode *inode)
{
	...
	/* Must return 0 to allow non-Sefs files to be looked up, too */
	return 0;
}
 ...
 */
static int sefs_lookup_interpose(struct dentry *dentry,
				     struct dentry *lower_dentry,
				     struct inode *dir_inode)
{
	...
	inode = __sefs_get_inode(lower_inode, dir_inode->i_sb);
	if (IS_ERR(inode)) {
...
/**
 * sefs_lookup
 * @sefs_dir_inode: The Sefs directory inode
 * @sefs_dentry: The Sefs dentry that we are looking up
 * @sefs_nd: nameidata; may be NULL
 *
				      ...
				      unsigned int flags)
{
	...
	lower_dentry = lookup_one_len(sefs_dentry->d_name.name,
				      lower_dir_dentry,
				      sefs_dentry->d_name.len);
	...
	rc = sefs_encrypt_and_encode_filename(
		...
		NULL, mount_crypt_stat, sefs_dentry->d_name.name,
		sefs_dentry->d_name.len);
	if (rc) {
...
interpose:
	rc = sefs_lookup_interpose(sefs_dentry, lower_dentry,
				       sefs_dir_inode);
out:
			 ...
			 struct dentry *new_dentry)
{
	...
	set_nlink(old_dentry->d_inode,
		  sefs_inode_to_lower(old_dentry->d_inode)->i_nlink);
...
{
	return sefs_do_unlink(dir, dentry, dentry->d_inode);
}
...
static void *sefs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	size_t len;
	char *buf = sefs_readlink_lower(dentry, &len);
	if (IS_ERR(buf))
		goto out;
	fsstack_copy_attr_atime(dentry->d_inode,
				sefs_dentry_to_lower(dentry)->d_inode);
	buf[len] = '\0';
out:
	nd_set_link(nd, buf);
	return NULL;
			  ...
			  struct iattr *lower_ia)
{
	...
	if (rc)
		return rc;
	crypt_stat = &sefs_inode_to_private(dentry->d_inode)->crypt_stat;
...
static int sefs_inode_newsize_ok(struct inode *inode, loff_t offset)
{
	...
	if (lower_newsize > lower_oldsize) {
		/*
		 * The Sefs inode and the new *lower* size are mixed here
 ...
 *
 * Simple function that handles the truncation of an Sefs inode and
...
int sefs_truncate(struct dentry *dentry, loff_t new_length)
{
	...
	rc = sefs_inode_newsize_ok(dentry->d_inode, new_length);
	if (rc)
		return rc;
...
static int sefs_setattr(struct dentry *dentry, struct iattr *ia)
{
	...
	if (!(crypt_stat->flags & SEFS_STRUCT_INITIALIZED))
		sefs_init_crypt_stat(crypt_stat);
...
static int sefs_getattr_link(struct vfsmount *mnt, struct dentry *dentry,
				 struct kstat *stat)
{
	struct sefs_mount_crypt_stat *mount_crypt_stat;
	int rc = 0;
...
static int sefs_getattr(struct vfsmount *mnt, struct dentry *dentry,
			    struct kstat *stat)
{
	...
	if (!rc) {
		fsstack_copy_attr_all(dentry->d_inode,
				      sefs_inode_to_lower(dentry->d_inode));
		generic_fillattr(dentry->d_inode, stat);
...
int
sefs_setxattr(struct dentry *dentry, const char *name, const void *value,
...
ssize_t
sefs_getxattr_lower(struct dentry *lower_dentry, const char *name,
...
static ssize_t
sefs_getxattr(struct dentry *dentry, const char *name, void *value,
		  size_t size)
{
	return sefs_getxattr_lower(sefs_dentry_to_lower(dentry), name,
				       value, size);
}
...
static int sefs_removexattr(struct dentry *dentry, const char *name)
{
	int rc = 0;
	struct dentry *lower_dentry;
