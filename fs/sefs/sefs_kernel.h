...
#define SEFS_MAX_CIPHER_NAME_SIZE 32
...
#define SEFS_I_SIZE_INITIALIZED   0x00004000
	...
	unsigned char cipher[SEFS_MAX_CIPHER_NAME_SIZE];
 ...
 *                         inode comes into existence, Sefs copies
 ...
 * given Sefs mount. This struct maintains a reference to both the
 ...
 * @key_tfm_mutex: Mutex to ensure only one operation in Sefs is
 ...
 *
 * Typically, Sefs will use the same ciphers repeatedly throughout
 * the course of its operations. In order to avoid unnecessarily
 * destroying and initializing the same cipher repeatedly, Sefs
...
extern struct mutex sefs_key_tfm_list_mutex;
...
void sefs_init_crypt_stat(struct sefs_crypt_stat *crypt_stat);
