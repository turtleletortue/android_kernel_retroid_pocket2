				  ...
				  struct sefs_auth_tok **auth_tok)
{
	...
	if (sefs_verify_version((*auth_tok)->version)) {
		printk(KERN_ERR "Data structure version mismatch. Userspace "
		       "tools must match Sefs kernel module with major "
 ...
 * Sefs. It encrypts the given filename with the mount-wide
			     ...
			     char *filename, size_t filename_size)
{
	...
	s->hash_desc.tfm = crypto_alloc_hash(SEFS_TAG_70_DIGEST, 0,
					     CRYPTO_ALG_ASYNC);
	...
	memcpy(&s->block_aligned_filename[s->num_rand_bytes], filename,
	       filename_size);
	rc = sefs_virt_to_scatterlist(s->block_aligned_filename,
	...
	rc = sefs_virt_to_scatterlist(&dest[s->i], s->block_aligned_filename_size,
				 s->dst_sg, 2);
	if (rc < 1) {
		printk(KERN_ERR "%s: Internal error whilst attempting to "
	 ...
	 * >= SEFS_MAX_IV_BYTES. */
	memset(s->iv, 0, SEFS_MAX_IV_BYTES);
	s->desc.info = s->iv;
...
struct sefs_parse_tag_70_packet_silly_stack {
	...
	char iv[SEFS_MAX_IV_BYTES];
	char cipher_string[SEFS_MAX_CIPHER_NAME_SIZE];
};
			     ...
			     char *data, size_t max_packet_size)
{
	...
	rc = sefs_get_tfm_and_mutex_for_cipher_name(&s->desc.tfm,
							&s->tfm_mutex,
							s->cipher_string);
	if (unlikely(rc)) {
		printk(KERN_ERR "Internal error whilst attempting to get "
	...
	mutex_lock(s->tfm_mutex);
	rc = sefs_virt_to_scatterlist(&data[(*packet_size)],
				 s->block_aligned_filename_size, s->src_sg, 2);
	if (rc < 1) {
		printk(KERN_ERR "%s: Internal error whilst attempting to "
		       "convert encrypted filename memory to scatterlist; "
	...
	rc = sefs_virt_to_scatterlist(s->decrypted_filename,
 ...
 * @auth_tok_list: Sefs parses packets into authentication tokens;
					 ...
					 struct sefs_crypt_stat *crypt_stat)
{
	...
	rc = sefs_get_tfm_and_mutex_for_cipher_name(&desc.tfm, &tfm_mutex,
							crypt_stat->cipher);
	if (unlikely(rc)) {
		printk(KERN_ERR "Internal error whilst attempting to get "
		       "tfm and mutex for cipher name [%s]; rc = [%d]\n",
	...
	rc = sefs_virt_to_scatterlist(auth_tok->session_key.encrypted_key,
	...
	auth_tok->session_key.decrypted_key_size =
		auth_tok->session_key.encrypted_key_size;
	rc = sefs_virt_to_scatterlist(auth_tok->session_key.decrypted_key,
				 auth_tok->session_key.decrypted_key_size,
				 dst_sg, 2);
	if (rc < 1 || rc > 2) {
		printk(KERN_ERR "Internal error whilst attempting to convert "
 ...
 * @sefs_dentry: The Sefs dentry associated with the packet set
 *
			      ...
			      struct dentry *sefs_dentry)
{
	...
	if (list_empty(&auth_tok_list)) {
		printk(KERN_ERR "The lower file appears to be a non-encrypted "
		       "Sefs file; this is not supported in this version "
		       "of the Sefs kernel module\n");
		rc = -EINVAL;
		goto out;
	}
		   ...
		   struct sefs_key_record *key_rec, size_t *packet_size)
{
	...
	rc = sefs_virt_to_scatterlist(crypt_stat->key, key_rec->enc_key_size,
				 src_sg, 2);
	if (rc < 1 || rc > 2) {
	...
	rc = sefs_virt_to_scatterlist(key_rec->enc_key, key_rec->enc_key_size,
				 dst_sg, 2);
	if (rc < 1 || rc > 2) {
		sefs_printk(KERN_ERR, "Error generating scatterlist "
