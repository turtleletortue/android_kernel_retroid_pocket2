 ...
 * Key Expansion
 */
static void key_expansion(unsigned char  *key, unsigned char  *w) {
...
int myaes_encrypt(unsigned char *in,unsigned char* out, int len)
{
	int nn = 0;
	unsigned char  key[] = {
		0x37, 0x31, 0x34, 0x39,
	...
	//sprintf(key,"%x%x%x%x",0x71491749,0x76401374,0x95812478,0x83616384);
	switch (sizeof(key)) {
		default:
	...
	w = kmalloc(Nb*(Nr+1)*4,GFP_KERNEL);
	...
	for(nn = 0; nn<num; nn++)
	{
		cipher(in + nn*16, out + nn*16, w);
	}
	kfree(w);
	return 0;
}

int myaes_decrypt(unsigned char *in,unsigned char* out, int len)
{
	int nn = 0;
	unsigned char  key[] = {
		0x37, 0x31, 0x34, 0x39,
	...
	for(nn = 0; nn<num; nn++)
	{
		inv_cipher(in + nn*16, out + nn*16, w);
	}
	kfree(w);
