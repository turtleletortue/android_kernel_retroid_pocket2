...
static int efs_parse(struct efs_dev *dev,unsigned char*buffer,int len)
{
	...
	// des dec
	dec_buffer = kmalloc(len, GFP_KERNEL);
	if(!dec_buffer)
		return -1;
	
	myaes_decrypt(buffer, dec_buffer, len);
	...
	{
		EFS_DEBUG("ecryptfs kill  start_magic:%x \n",start_magic);
		//TRACE;
...
static int efs_node_check_vaild(struct check_list *node)
{
	...
	//printk("ecryptfs valid_check_resule:%d    crc:%x    node->crc32:%x\n",valid_check_resule,crc,node->crc32);
...
static void efs_self_cheak(struct efs_dev *dev)
{
	...
	txt = get_txt(path);
	//printk("ecryptfs efs_self_cheak %s  %x\n",txt,path);
...
static void efs_check_valid_start(struct efs_dev *dev)
{
	...
	{
		node = list_entry(plist, struct check_list, list);
		if(strcmp(node->f_name,"debug_mode_v1") == 0)
		{
			EFS_DEBUG("ecryptfs this is debug\n");
			dev->debug_mode = DEBUG_MODE;
			return;
		}
	}
	
	//printk("<1>ecryptfs start\n");
	list_for_each_safe(plist,n,head)
	{
		node = list_entry(plist, struct check_list, list);
		if(node->valid_check)
		{
			...
			if(ret != 1)
			{
				EFS_DEBUG("ecryptfs check %s fail\n",node->f_name);
				dev->check_valid_result = CHECK_FAIL;
				return;
			}
		}
...
int efs_check_path(struct list_head * cl_head,struct list_head * pm_head)
{
	...
	//printk("ecryptfs efs_check_path:%s\n",txt);
	if(txt[0] == '\0')
	{
		kfree(txt);
		return -1;
	...
	else  // guess maybe apk
	{
		list_for_each_safe(plist,n,pm_head)
		{
			...
			//printk("ecryptfs efs_check_path  guess maybe apk,checking: %s\n",pm_node->package_name);
		...
		if(abs_path == 0)
		{
			///printk("ecryptfs efs_check_path  %s not absolute path or apk\n",txt);
			kfree(txt);
			return -1;
		}
	}
	...
	list_for_each_safe(plist,n,cl_head)
	{
		...
		{
			//printk("ecryptfs efs_check_path  allow:%s\n",abs_path);
			kfree(txt);
			return 0;
		}
	}

	//printk("ecryptfs efs_check_path  not allow:%s\n",abs_path);
	kfree(txt);
	return -1;
}
...
static int efs_clean_old_node(struct efs_dev *dev,int *pid,int count)
{
	...
	//for(i = 0; i<count; i++)
	//{
	//	printk("ecryptfs new ps list pid[%d]:%d\n",i,pid[i]);
	//}
	...
	{
		node = list_entry(plist, struct ac_list, list);
		//printk("ecryptfs clean pid:%d\n",node->pid);
		list_del(&node->list);
		kfree((void*)node);
	}

	//printk("ecryptfs  auth_count:%d\n",auth_count);
...
static int efs_open(struct inode *inode, struct file *filp)
{
	...
	//printk("ecryptfs efs_open (pid=%d, comm=%s)\n", current->pid, current->comm);
	return 0;          /* success */
}
                ...
                loff_t *f_pos)
{
	...
	printk(KERN_ERR "dywu efs_write !!!!!!!!!!!!!!!!!!!\n");
      ...
      case EFS_IOC_START_CHECK: 
      	{
			...
			{
				EFS_DEBUG("ecryptfs check valid fail\n");
				return -ENOMEM;
			}
			...
			if(dev->debug_mode == 0 && dev->check_valid_result != CHECK_PASS)
			{
				EFS_DEBUG("ecryptfs self cheak fail\n");
				return -ENOMEM;
			}
			kfree(dev->key_buffer);
			dev->key_len = -1;
	  ...
	  case EFS_IOC_SET_PM_LIST:
      	{
			...
			//printk("ecryptfs EFS_IOC_SET_PM_LIST  %s  %s\n",node->package_name,node->absolute_path);
      	}
        break;
			       ...
			       const char *buff, size_t size)
{
	...
	//printk("ecryptfs debug_store:%s len:%ld keyfile_size:%d\n",b,len,keyfile_size);
	return size;
}
...
#if 0
static struct device_attribute *ecryptfs_attributes[] = {
	&dev_attr_value,
	NULL
};
...
int efs_init_module(void)
{
	...
	dev_t dev = 0;
	struct efs_dev * pdev;
	//struct device_attribute **attrs = ecryptfs_attributes;
	//struct device_attribute *attr;
	//int err;
	
	printk("KERN_ERR dywu efs_init_module\n");
	...
	efs_class = class_create(THIS_MODULE, "ecryptfs");
...
#endif	
	printk(KERN_WARNING "ecryptfs efs_major:%d\n", efs_major);
	return 0; /* succeed */
